...
{
    // Create program kernel(s)
    this->_program.createKernels(&this->_kernels);
    std::cout << "Num kernels: " << this->_kernels.size() << '\n';
    for (cl_uint i=0; i<this->_kernels.size(); ++i)
    {
      std::cout << "=> Kernel # " << i << ": " << this->_kernels[i].getInfo<CL_KERNEL_FUNCTION_NAME>() << '\n';
    }
    std::cout << std::endl;
    return CL_SUCCESS;
}
...
cl::Kernel* ClManager::getKernel(const std::string &name)
{
    ...
    for (cl_uint i=0; i<this->_kernels.size(); ++i)
    {
        ...
        kernel_name = kernel.getInfo<CL_KERNEL_FUNCTION_NAME>();
